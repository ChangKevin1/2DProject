...
public class ScrollingScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (target != null)
        {
            distance = distance + speed.x * direction.x;
            movement = new Vector3(
              Mathf.Lerp(transform.position.x, target.transform.position.x + distance, followSpeed),
              Mathf.Lerp(transform.position.y, target.transform.position.y - verticalDistance , followSpeed),
